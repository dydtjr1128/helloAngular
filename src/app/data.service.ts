...
export class DataService {
  ...
  getCustomersByLastName(lastName: string): Promise<Customer[]> {//Promise 형태로 리턴
    ...
    return this.http.get(url)
      ...
      .then(response => response.json() as Customer[])//primise가 성공해서 return된 경우 response가 넘어온다. Customer라는 변수에 저장한다.
  ...
  create(customer: Customer): Promise<Customer> {
    ...
    return this.http
      .post(this.baseUrl, JSON.stringify(customer), {headers : this.headers})//customer라는 객체를 serization직렬화 한다.
      .toPromise()
